...
def ChiSquareDrift(x_ref, x, threshold: float = .05, return_p_val=True, return_distance=True):
    p_vals, dist = feature_score_Chi(x_ref, x)
    threshold = threshold
...
def feature_score_KS(x_ref, x):
    x_ref = entropy(softmax(x_ref.detach().numpy(), axis=-1), axis=-1)
...
def KSDrift(x_ref, x, threshold: float = .05, return_p_val=True, return_distance=True):
    p_vals, dist = feature_score_KS(x_ref, x)
    threshold = threshold
    drift_pred = int((p_vals < threshold).any())  # type: ignore[assignment]
    cd = {}
    cd['is_drift'] = drift_pred
...
def drift_detection(x_ref, x, threshold: float = .05, 
                    return_p_val=True, return_distance=True, method='KSDrift'):
    if method == 'KSDrift':
        return KSDrift(x_ref, x, threshold)
    elif method == "ChiSquareDrift":
        return ChiSquareDrift(x_ref, x, threshold)